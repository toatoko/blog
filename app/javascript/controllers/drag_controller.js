...
/**
 * The data attribute containg the draggable element's outer parent
...
let resourceID;
/**
 * The draggable element's new position. In JS this is 0 based
...
export default class extends Controller {
  ...
  dragStart(event) {
    ...
    console.log("Dragging item ID:", resourceID);
    event.dataTransfer.effectAllowed = "move";
  }
  /** 
   * Called when the user dragging an element over another one and releases
   * the mouse button
   * 
   * @param {*} event The drag event, which can be used to access the dragged element
   */
  ...
  /** 
   * Called when the user drag an element over another element
   * @param {*} event The drag event, which can be used to access the dragged element
   */
  dragOver(event) {
    event.preventDefault();
  ...
  /** 
   * Called when the mouse is dragged over anothe element
   * @param {*} event The drag event, which can be used to access the dragged element
   */
  dragEnter(event) {
    event.preventDefault();
  ...
  /**
   * Attempts to find the appropiate draggable element in order to parent 
   ...
   * @param {*} parentID The ID of the parent element. This is used to prevent traversing upo to the document root.
   * @returns The elemnt that the dragged elemnt should be parented to. 
   */
  ...
  /**
   * Sets the new position of the dragged elemnt. Positions in the JS
   * controller are 0 based, but the positions in the Rails gem are 1 based.
   * 
   * So this index in incremented by 1 on the server side after it is sent over,
   * to match the ID.
   * 
  ...
  /**
   * Gets the value of the meta tag with the given name. This is used to get the CSRF token.
   * 
